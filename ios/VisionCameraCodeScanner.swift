...
class VisionCameraCodeScanner: NSObject, FrameProcessorPluginBase {
    ...
    private static var isProcessingFallback: Bool? = false
    private static var zxingBarcodeReader: ZXIBarcodeReader? = nil
    private static var zxingBarcodeReaderOptions = ZXIReaderOptions()
    

    static func initializeZXingReaderIfNeeded() {
        if zxingBarcodeReader == nil {
            ...
            zxingBarcodeReaderOptions.formats = [
                1,     // CODE_128
                32,    // EAN_13
                512,   // UPC_A
                1024,  // UPC_E
                2048,  // RSS_14 (Databar)
                4096   // RSS_EXPANDED (Databar Expanded)
            ]

            zxingBarcodeReader = ZXIBarcodeReader(options: zxingBarcodeReaderOptions)
        }
    }


    public static func callback(_ frame: Frame!, withArgs args: [Any]!) -> Any! {
        print("callback pf vision camera code scanner SWIFT")
        Self.initializeZXingReaderIfNeeded()
...
//         print("------END VisionCameraCodeScanner--------")
       ...
       if args.count > 0, let types = args[0] as? [Any] {
            barcodeType = types
        }

        guard args.count > 1, let options = args[1] as? [String: Any] else {
            return [:]
        }
        ...
        if let documentDirectory = getDocumentsDirectory(){
            ...
            print("absoluteModelPath: \(absoluteModelPath)")
            if !FileManager.default.fileExists(atPath: absoluteModelPath) {
                print("Model file NOT FOUND at path: \(absoluteModelPath)")
            }
        }
        do {
            ...
            if checkInverted {
                ...
                barcodes.append(contentsOf: try Self.barcodeScanner!.results(in: VisionImage(image: invertedImage)))
            }
            print("trying if block:")
            if !barcodes.isEmpty {
            ...
            print("Invoking BarcodeDetectorHelper")

            let BarcodeDetectorHelper = BarcodeDetectorHelper(
                modelPath: absoluteModelPath,
                modelName: modelName,
                scoreThreshold: Float(threshold),
                maxResults: 3,
                mImageSize: modelImageSize!
            )
            print("After BarcodeDetectorHelper")

            guard let visionImageBuffer = uiImageToCVPixelBuffer(image: image) else { return [] }
            print("After visionImageBuffer")

            do {
                 print("calling BarcodeDetectorHelper?.detect")
                guard let pluCodeMarkers = try BarcodeDetectorHelper?.detect(pixelBuffer: visionImageBuffer) as? [[String: Any]] else {
                    print("pluCodeMarkers is nil or in unexpected format")
                    return []
                }
                print("after calling BarcodeDetectorHelper?.detect")
                var outputObjectIndex = 1


                if !pluCodeMarkers.isEmpty && !(self.isProcessingFallback ?? false) {
                     print("in pluCodeMarkers if block..")
                    Self.isProcessingFallback = true
                     print("after isProcessingFallback")
                    var tempResults = Set<String>()
                     print("after tempResults")
                    let zxingReader = Self.zxingBarcodeReader!
                    print("after Self.zxingBarcodeReader!")

                    for detection in pluCodeMarkers {
                         print("in pluCodeMarkers for block")
                        guard
                        ...
                        print("detectMarkerOnly \(detectMarkerOnly)")
                        if !detectMarkerOnly {               

                            print("ENTERED TEST IF BLOCK")
                            for k in 0..<2 {
...
//                                }
        ...
        print("barCodeAttributes at end: \(barCodeAttributes)")
        return barCodeAttributes
    }
    
    // Helper function to map ZXing format string to your app's integer codes
    static func mapZxingFormatStringToInteger(_ formatString: String) -> String {
    ...
    static func ciImageToGrayscaleBytes(_ ciImage: CIImage) -> (bytes: [UInt8], width: Int, height: Int)? {
    ...
    static func calculateBoundingBox(from points: [CGPoint]) -> (minX: CGFloat, minY: CGFloat, maxX: CGFloat, maxY: CGFloat) {
    ...
    static func saveImageToDocuments(_ image: UIImage, fileName: String = "rotated.jpg") -> URL? {
    ...
    static func rotate(bitmap: UIImage, byDegrees degrees: CGFloat, imgIndex: Int) -> UIImage {
       ...
       if let savedURL = Self.saveImageToDocuments(rotatedImage!, fileName: "tj-rotated-\(imgIndex).jpg") {
    ...
    static func createScanner(_ args: [Any]!) throws {
        print("createScanner STARTED.......")
        print("Args received in createScanner: \(String(describing: args))")

        guard let rawFormats = args as? [Int] else {
            print("THROWING noBarcodeFormatProvided ERROR")
            throw BarcodeError.noBarcodeFormatProvided
        }

        print("....Barcode formats:.... \(rawFormats)")
